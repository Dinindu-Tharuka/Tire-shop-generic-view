...
from stock_data.models import StockItem, StockItemsInvoice, StockItemUnique, StockPayment, Voucher
...
class StockItemList(ListAPIView):
    ...
    def get_queryset(self):
        stockItemsInvoiceNoFilter = self.request.GET.get(
            'stockItemsInvoiceNoFilter')
        ...
        stockItemsStartDateFilter = self.request.GET.get(
            'stockItemsStartDateFilter')
        stockItemsEndDateFilter = self.request.GET.get(
            'stockItemsEndDateFilter')
        ...
        if stockItemsInvoiceNoFilter or stockItemsItemIdFilter or stockItemsBrandFilter or stockItemsSizeFilter or stockItemsStartDateFilter or stockItemsEndDateFilter:
            ...
            if stockItemsInvoiceNoFilter:
                queryset = queryset.filter(
                    stock_invoice__invoice_no__istartswith=stockItemsInvoiceNoFilter)
            if stockItemsItemIdFilter:
                queryset = queryset.filter(
                    item__item_id__istartswith=stockItemsItemIdFilter)
            if stockItemsBrandFilter:
                queryset = queryset.filter(
                    item__brand__istartswith=stockItemsBrandFilter)
            if stockItemsSizeFilter:
                queryset = queryset.filter(
                    item__size__istartswith=stockItemsSizeFilter)
            if stockItemsStartDateFilter or stockItemsEndDateFilter:
                date_object = ConvertDateToDateTime(
                    stockItemsStartDateFilter, stockItemsEndDateFilter)
                queryset = queryset.filter(date__range=(
                    date_object.converted_min(), date_object.converted_max()))
        else:
            queryset = StockItem.objects.all()
        return queryset
...
class StockPageItemList(ListAPIView):
    ...
    def get_queryset(self):
        pageStockItemsInvoiceNoFilter = self.request.GET.get(
            'pageStockItemsInvoiceNoFilter')
        pageStockItemsItemIdFilter = self.request.GET.get(
            'pageStockItemsItemIdFilter')
        pageStockItemsBrandFilter = self.request.GET.get(
            'pageStockItemsBrandFilter')
        pageStockItemsSizeFilter = self.request.GET.get(
            'pageStockItemsSizeFilter')
        pageStockItemsStartDateFilter = self.request.GET.get(
            'pageStockItemsStartDateFilter')
        pageStockItemsEndDateFilter = self.request.GET.get(
            'pageStockItemsEndDateFilter')
        ...
        if pageStockItemsInvoiceNoFilter or pageStockItemsItemIdFilter or pageStockItemsBrandFilter or pageStockItemsSizeFilter or pageStockItemsStartDateFilter or pageStockItemsEndDateFilter:
            ...
            if pageStockItemsInvoiceNoFilter:
                queryset = queryset.filter(
                    stock_invoice__invoice_no__istartswith=pageStockItemsInvoiceNoFilter)
            if pageStockItemsItemIdFilter:
                queryset = queryset.filter(
                    item__item_id__istartswith=pageStockItemsItemIdFilter)
            if pageStockItemsBrandFilter:
                queryset = queryset.filter(
                    item__brand__istartswith=pageStockItemsBrandFilter)
            if pageStockItemsSizeFilter:
                queryset = queryset.filter(
                    item__size__istartswith=pageStockItemsSizeFilter)
            if pageStockItemsStartDateFilter or pageStockItemsEndDateFilter:
                date_object = ConvertDateToDateTime(
                    pageStockItemsStartDateFilter, pageStockItemsEndDateFilter)
                queryset = queryset.filter(date__range=(
                    date_object.converted_min(), date_object.converted_max()))
        else:
            queryset = StockItem.objects.all()
...
class AllStockItemsInvoiceList(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if filterGrnNo or filterInvoiceNo or filterSupplier:
            ...
            if filterInvoiceNo:
                queryset = queryset.filter(
                    bill_invoice_no__startswith=filterInvoiceNo)
            if filterSupplier:
...
class StockItemsInvoiceList(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        invoiceStockSupplierFilter = self.request.GET.get(
            'invoiceStockSupplierFilter')
        if query or invoiceBillIdFilter or invoiceStockSupplierFilter:
            ...
            if invoiceBillIdFilter:
                queryset = queryset.filter(
                    bill_invoice_no__startswith=invoiceBillIdFilter)
            if invoiceStockSupplierFilter:
                queryset = queryset.filter(
                    supplier__id=int(invoiceStockSupplierFilter))
        else:
            queryset = StockItemsInvoice.objects.all()
        return queryset
...
class StockItemsInvoiceDetail(RetrieveUpdateDestroyAPIView):
    ...
    def delete(self, request, pk):
        stock_ids = StockItem.objects.filter(
            stock_item_invoice__invoice_no=pk).values('id').distinct()
